...
function renderGlobalSearchResults(messages, query) {
    ...
    messages.forEach(msg => {
        ...
        html += `
            <div class="card mb-2 border shadow-sm" style="cursor: pointer; transition: transform 0.2s;" role="button" tabindex="0" onmouseover="this.style.transform='translateY(-2px)'" onmouseout="this.style.transform='translateY(0)'" onclick="loadGlobalResult('${msg.normalized_address}', '${msg.source}-${msg.id}')" onkeydown="if(event.key==='Enter' || event.key===' '){event.preventDefault(); loadGlobalResult('${msg.normalized_address}', '${msg.source}-${msg.id}')}">
    ...
    try {
        ...
        for (const u of users) {
            ...
            tr.innerHTML = `
                ...
                <td class="text-end">
                    <button class="btn btn-sm btn-outline-danger btn-delete-user" data-id="${u.id}" aria-label="Delete user"><i class="fas fa-trash"></i></button>
                </td>
            `;
